...
void database_print(struct info *child)
{
    ...
    for (i = 0; i < child_num; i++)
        if (child[i].hospital_cure)
            printf
                ("%d: %s was in hospital #%s with address %s, Doctor:%s, illness:%s\n",
                 i, child[i].child_surname, child[i].x.was.hosp_num,
                 child[i].x.was.hosp_address,
                 child[i].x.was.surname_hosp_doc, child[i].last_illness);
        else
            printf("%d: %s was not in hospital, Doc:%s illness:%s\n", i,
                   child[i].child_surname,
                   child[i].x.was_not.surname_distr_doc,
...
{
    child[0] = (struct info) {
    .last_illness = "fg",.child_surname = "Alehin",.hospital_cure = 1};
    child[0].x.was = (struct st2) {
    .hosp_num = "hg",.hosp_address = "hg",.surname_hosp_doc = "hg"};

    child[1] = (struct info) {
    .last_illness = "ospa",.child_surname = "Ivanov",.hospital_cure = 0};
    child[1].x.was_not = (struct st1) {
    .surname_distr_doc = "hg"};

    child[2] = (struct info) {
    .last_illness = "fg",.child_surname = "Smith",.hospital_cure = 1};
    child[2].x.was = (struct st2) {
    .hosp_num = "hg",.hosp_address = "hg",.surname_hosp_doc = "hg"};

    child[3] = (struct info) {
    .last_illness = "fg",.child_surname = "Petrov",.hospital_cure = 0};
    child[3].x.was_not = (struct st1) {
    .surname_distr_doc = "hg"};

    child[4] = (struct info) {
    .last_illness = "ospa",.child_surname = "Arib",.hospital_cure = 1};
    child[4].x.was = (struct st2) {
    .hosp_num = "hg",.hosp_address = "hg",.surname_hosp_doc = "hg"};
}
